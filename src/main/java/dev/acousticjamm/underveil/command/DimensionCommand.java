...
package dev.acousticjamm.underveil.command;
...
import dev.acousticjamm.underveil.procedures.DimensionCommandExecutedProcedure;
...
public class DimensionCommand {
	...
	public static void registerCommands(RegisterCommandsEvent event) {
		event.getDispatcher()
				.register(Commands.literal("dimension").requires(s -> s.hasPermission(4))
	...
	private static int execute(CommandContext<CommandSourceStack> ctx) {
		...
		DimensionCommandExecutedProcedure.execute(world, x, z, entity, cmdparams);
		return 0;
	}
}
