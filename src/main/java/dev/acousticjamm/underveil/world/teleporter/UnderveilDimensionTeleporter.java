...
public class UnderveilDimensionTeleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("underveil_dimension_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("underveil_dimension_portal",
				Sets.newHashSet(ImmutableSet.copyOf(UnderveilModBlocks.UNDERVEIL_DIMENSION_PORTAL.getStateDefinition().getPossibleStates())), 0, 1)
						.setRegistryName("underveil_dimension_portal");
	...
	public UnderveilDimensionTeleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
			...
			for (int i3 = -1; i3 < 2; ++i3) {
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0 ? UnderveilModBlocks.VEILSTONE.defaultBlockState() : Blocks.AIR.defaultBlockState();
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, UnderveilModBlocks.VEILSTONE.defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = UnderveilModBlocks.UNDERVEIL_DIMENSION_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getPortalRepositioner(entity, blockpos1).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = UnderveilDimensionPortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return UnderveilDimensionPortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
