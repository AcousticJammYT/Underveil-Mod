...
public class VeilTempleFeature extends Feature<NoneFeatureConfiguration> {
	public static final VeilTempleFeature FEATURE = (VeilTempleFeature) new VeilTempleFeature().setRegistryName("underveil:veil_temple");
	public static final ConfiguredFeature<?, ?> CONFIGURED_FEATURE = FEATURE.configured(FeatureConfiguration.NONE);
	public static final Set<ResourceLocation> GENERATE_BIOMES = null;
	private StructureTemplate template = null;

	public VeilTempleFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		if (!dimensionCriteria)
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("underveil", "veil_temple"));
		if (template == null)
			return false;
