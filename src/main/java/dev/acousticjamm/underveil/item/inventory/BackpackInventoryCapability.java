...
package dev.acousticjamm.underveil.item.inventory;
...
public class BackpackInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	...
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == UnderveilModItems.BACKPACK) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(8) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != UnderveilModItems.BACKPACK;
			}
